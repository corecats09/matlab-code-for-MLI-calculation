...
for n = 1:15
lnLmax_sta(:,n) = n.*log(sqrt(2.*pi).*std_residual_sta(1,n))- norm(residual_sta(:,n)).^2./2./(std_residual_sta(1,n).^2);
end
for n = 1:15
cov_temp =std_residual_sta(1,n)^2./(length(mean_log)-length(n)) * inv(tbl_ele_adj(:,1:n)'* tbl_ele_adj(:,1:n));
det_cov_sta(1,n) = det(cov_temp);
end
for n = 1:15
lnMLI_sta(1,n) = n/2.*log(2*pi)+lnLmax_sta(1,n)+0.5.*log(det_cov_sta(1,n))-n.*log(2500);
end
for n = 1:15
...
for n = 1:15
lnLmax_log(:,n) = n.*log(sqrt(2.*pi).*std_residual_log(1,n))-norm(residual_log(:,n)).^2./2./(std_residual_log(1,n).^2);
end
for n = 1:15
cov_temp =std_residual_log(1,n)^2./(length(mean_sta)-length(n))  * inv(tbl_ele_adj(:,1:n)'* tbl_ele_adj(:,1:n));
